...
#--------------------------------------------------------------------------------#
#2013년 확정자료 
setwd("D:\\EUMC\\데이터관리\\Mornitoring_data\\Air_Korea_확정데이터\\2013")

air_q1<-read_excel("2013년01분기.xlsx",sheet=1) ; air_q1<- as.data.frame(air_q1)
air_q2<-read_excel("2013년02분기.xlsx",sheet=1) ; air_q2<- as.data.frame(air_q2)
air_q3<-read_excel("2013년03분기.xlsx",sheet=1) ; air_q3<- as.data.frame(air_q3)
air_q4<-read_excel("2013년04분기.xlsx",sheet=1) ; air_q4<- as.data.frame(air_q4)

nrow(air_q1) ; nrow(air_q2) ;nrow(air_q3) ;nrow(air_q4)

#data merge 
air2013<-rbind(air_q1,air_q2,air_q3,air_q4)

#colume 명 변경 
names(air2013)[c(1:4,10)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2013$yymmdd=substr(air2013$datetime,1,8) #년월일 

#하루 기준 75% 이상 측정만 추출하기 위한 merge key 
air2013$mkey=paste0(air2013$code,"-",air2013$yymmdd)
...
air2013.r<-sqldf("select * from air2013 where code in (select code from code)")
air2013.r<-subset(air2013,code %in% cd)

air2013.r$yymmdd=ymd(air2013.r$yymmdd) # 날짜형식으로변경 
daily=data.frame(yymmdd=seq(as.Date("2013-01-01"),as.Date("2013-12-31"),by="day"))

#--------------------------------------------------------------------------------#
#측정소별 AP 산출 
ap_list=NULL
key<-unique(air2013.r$code)
for(i in 1:length(key)) {
  dat<-subset(air2013.r,code==key[i])
  a<-sqldf("select mkey,count(SO2) as so2_count,count(SO2) as co_count, count(O3) as o3_count,
  ...
  #2013-01-01 ~ 2013-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
