...
#--------------------------------------------------------------------------------#
#2010년 확정자료 
setwd("D:\\EUMC\\데이터관리\\Mornitoring_data\\Air_Korea_확정데이터\\2010")

air_q1<-read_excel("2010년01분기.xlsx",sheet=1) ; air_q1<- as.data.frame(air_q1)
air_q2<-read_excel("2010년02분기.xlsx",sheet=1) ; air_q2<- as.data.frame(air_q2)
air_q3<-read_excel("2010년03분기.xlsx",sheet=1) ; air_q3<- as.data.frame(air_q3)
air_q4<-read_excel("2010년04분기.xlsx",sheet=1) ; air_q4<- as.data.frame(air_q4)

nrow(air_q1) ; nrow(air_q2) ;nrow(air_q3) ;nrow(air_q4)

#data merge 
air2010<-rbind(air_q1,air_q2,air_q3,air_q4)

#colume 명 변경 
names(air2010)[c(1:4,10)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2010$yymmdd=substr(air2010$datetime,1,8) #년월일 

#하루 기준 75% 이상 측정만 추출하기 위한 merge key 
air2010$mkey=paste0(air2010$code,"-",air2010$yymmdd)
...
air2010.r<-sqldf("select * from air2010 where code in (select code from code)")
air2010.r<-subset(air2010,code %in% cd)

air2010.r$yymmdd=ymd(air2010.r$yymmdd) # 날짜형식으로변경 
daily=data.frame(yymmdd=seq(as.Date("2010-01-01"),as.Date("2010-12-31"),by="day"))

#--------------------------------------------------------------------------------#
#측정소별 AP 산출 
ap_list=NULL
key<-unique(air2010.r$code)
for(i in 1:length(key)) {
  dat<-subset(air2010.r,code==key[i])
  ...
  #2010-01-01 ~ 2010-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
