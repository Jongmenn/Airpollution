...
#--------------------------------------------------------------------------------#
#2012년 확정자료 
setwd("D:\\EUMC\\데이터관리\\Mornitoring_data\\Air_Korea_확정데이터\\2012")

air_q1<-read_excel("2012년01분기.xlsx",sheet=1) ; air_q1<- as.data.frame(air_q1)
air_q2<-read_excel("2012년02분기.xlsx",sheet=1) ; air_q2<- as.data.frame(air_q2)
air_q3<-read_excel("2012년03분기.xlsx",sheet=1) ; air_q3<- as.data.frame(air_q3)
air_q4<-read_excel("2012년04분기.xlsx",sheet=1) ; air_q4<- as.data.frame(air_q4)

nrow(air_q1) ; nrow(air_q2) ;nrow(air_q3) ;nrow(air_q4)

#data merge 
air2012<-rbind(air_q1,air_q2,air_q3,air_q4)

#colume 명 변경 
names(air2012)[c(1:4,10)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2012$yymmdd=substr(air2012$datetime,1,8) #년월일 

#하루 기준 75% 이상 측정만 추출하기 위한 merge key 
air2012$mkey=paste0(air2012$code,"-",air2012$yymmdd)
...
air2012.r<-sqldf("select * from air2012 where code in (select code from code)")
air2012.r<-subset(air2012,code %in% cd)

air2012.r$yymmdd=ymd(air2012.r$yymmdd) # 날짜형식으로변경 
daily=data.frame(yymmdd=seq(as.Date("2012-01-01"),as.Date("2012-12-31"),by="day"))

#--------------------------------------------------------------------------------#
#측정소별 AP 산출 
ap_list=NULL
key<-unique(air2012.r$code)
for(i in 1:length(key)) {
  dat<-subset(air2012.r,code==key[i])
  ...
  #2012-01-01 ~ 2012-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
