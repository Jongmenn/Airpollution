#2020년 확정자료 
setwd("D:\\EUMC\\데이터관리\\Mornitoring_data\\Air_Korea_확정데이터\\2020")

library(readxl)   ;library(sqldf)
library(lubridate);library(ggplot2) ;library(gridExtra)
list.files()
air_q1  <-read_excel("2020년 1월.xlsx",sheet=1)  ; air_q1<- as.data.frame(air_q1)
air_q2  <-read_excel("2020년 2월.xlsx",sheet=1)  ; air_q2<- as.data.frame(air_q2)
air_q3  <-read_excel("2020년 3월.xlsx",sheet=1)  ; air_q3<- as.data.frame(air_q3)
air_q4  <-read_excel("2020년 4월.xlsx",sheet=1)  ; air_q4<- as.data.frame(air_q4)
air_q5  <-read_excel("2020년 5월.xlsx",sheet=1)  ; air_q5<- as.data.frame(air_q5)
air_q6  <-read_excel("2020년 6월.xlsx",sheet=1)  ; air_q6<- as.data.frame(air_q6)
air_q7  <-read_excel("2020년 7월.xlsx",sheet=1)  ; air_q7<- as.data.frame(air_q7)
air_q8  <-read_excel("2020년 8월.xlsx",sheet=1)  ; air_q8<- as.data.frame(air_q8)
air_q9  <-read_excel("2020년 9월.xlsx",sheet=1)  ; air_q9<- as.data.frame(air_q9)
air_q10 <-read_excel("2020년 10월.xlsx",sheet=1)  ; air_q10<- as.data.frame(air_q10)
air_q11 <-read_excel("2020년 11월.xlsx",sheet=1)  ; air_q11<- as.data.frame(air_q11)
air_q12 <-read_excel("2020년 12월.xlsx",sheet=1)  ; air_q12<- as.data.frame(air_q12)

#data merge 
air2020<-rbind(air_q1,air_q2,air_q3,air_q4,air_q5,air_q6,air_q7,air_q8,air_q9,air_q10,air_q11,air_q12)
air2020 <- air2020  %>%  filter(망=="도시대기") %>% select(-망)
names(air_2020)


#colume 명 변경 
names(air2020)[c(1:4,11)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2020$yymmdd=substr(air2020$datetime,1,8) #년월일 

#하루 기준 75% 이상 측정만 추출하기 위한 merge key 
air2020$mkey=paste0(air2020$code,"-",air2020$yymmdd)
...
air2020.r<-sqldf("select * from air2020 where codename in (select yr2020 from code)")

# air2020.r<-subset(air2020,code %in% yr2020)

air2020.r$yymmdd=ymd(air2020.r$yymmdd) # 날짜형식으로변경 
daily=data.frame(yymmdd=seq(as.Date("2020-01-01"),as.Date("2020-12-31"),by="day"))

#--------------------------------------------------------------------------------#
#측정소별 AP 산출 
ap_list=NULL
key<-unique(air2020.r$code)
for(i in 1:length(key)) {
  dat<-subset(air2020.r,code==key[i])
  ...
  #2020-01-01 ~ 2020-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
