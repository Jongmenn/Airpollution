...
#--------------------------------------------------------------------------------#
#2017년 확정자료 
setwd("D:\\EUMC\\데이터관리\\Mornitoring_data\\Air_Korea_확정데이터\\2017")

air_q1<-read_excel("2017년 1분기.xlsx") ; air_q1<- as.data.frame(air_q1)
air_q2<-read_excel("2017년 2분기.xlsx") ; air_q2<- as.data.frame(air_q2)
air_q3<-read_excel("2017년 3분기.xlsx") ; air_q3<- as.data.frame(air_q3)
air_q4<-read_excel("2017년 4분기.xlsx") ; air_q4<- as.data.frame(air_q4)

nrow(air_q1) ; nrow(air_q2) ;nrow(air_q3) ;nrow(air_q4)

#data merge 
air2017<-rbind(air_q1,air_q2,air_q3,air_q4) %>% select(지역,측정소명,측정소코드,측정일시:주소)

#colume 명 변경 
names(air2017)[c(1:4,11)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2017$yymmdd=substr(air2017$datetime,1,8) #년월일 

#하루 기준 75% 이상 측정만 추출하기 위한 merge key 
air2017$mkey=paste0(air2017$code,"-",air2017$yymmdd)
...
air2017.r<-sqldf("select * from air2017 where code in (select code from code)")
air2017.r<-subset(air2017,code %in% cd)

air2017.r$yymmdd=ymd(air2017.r$yymmdd) # 날짜형식으로변경 
daily=data.frame(yymmdd=seq(as.Date("2017-01-01"),as.Date("2017-12-31"),by="day"))

#--------------------------------------------------------------------------------#
#측정소별 AP 산출 
ap_list=NULL
key<-unique(air2017.r$code)
for(i in 1:length(key)) {
  dat<-subset(air2017.r,code==key[i])
  ...
  #2017-01-01 ~ 2017-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
