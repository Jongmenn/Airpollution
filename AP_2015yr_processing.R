...
#--------------------------------------------------------------------------------#
#2015년 확정자료 
setwd("D:\\EUMC\\데이터관리\\Mornitoring_data\\Air_Korea_확정데이터\\2015")

air_q1<-read.csv("2015년 1분기.csv") ; air_q1<- as.data.frame(air_q1)
air_q2<-read.csv("2015년 2분기.csv") ; air_q2<- as.data.frame(air_q2)
air_q3<-read.csv("2015년 3분기.csv") ; air_q3<- as.data.frame(air_q3)
air_q4<-read.csv("2015년 4분기.csv") ; air_q4<- as.data.frame(air_q4)

nrow(air_q1) ; nrow(air_q2) ;nrow(air_q3) ;nrow(air_q4)

#data merge 
air2015<-rbind(air_q1,air_q2,air_q3,air_q4) %>% select(지역,측정소명,측정소코드,측정일시:주소)

#colume 명 변경 
names(air2015)[c(1:4,11)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2015$yymmdd=substr(air2015$datetime,1,8) #년월일 

#하루 기준 75% 이상 측정만 추출하기 위한 merge key 
air2015$mkey=paste0(air2015$code,"-",air2015$yymmdd)
...
air2015.r<-sqldf("select * from air2015 where code in (select code from code)")
air2015.r<-subset(air2015,code %in% cd)

air2015.r$yymmdd=ymd(air2015.r$yymmdd) # 날짜형식으로변경 
daily=data.frame(yymmdd=seq(as.Date("2015-01-01"),as.Date("2015-12-31"),by="day"))

#--------------------------------------------------------------------------------#
#측정소별 AP 산출 
ap_list=NULL
key<-unique(air2015.r$code)
for(i in 1:length(key)) {
  dat<-subset(air2015.r,code==key[i])
  ...
  #2015-01-01 ~ 2015-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
