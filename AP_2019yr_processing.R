...
air_q3  <-read_excel("2019년 3월.xlsx",sheet=1)  ; air_q3<- as.data.frame(air_q3)
air_q4  <-read_excel("2019년 4월.xlsx",sheet=1)  ; air_q4<- as.data.frame(air_q4)
air_q5  <-read_excel("2019년 5월.xlsx",sheet=1)  ; air_q5<- as.data.frame(air_q5)
air_q6  <-read_excel("2019년 6월.xlsx",sheet=1)  ; air_q6<- as.data.frame(air_q6)
...
#colume 명 변경 
names(air2019)[c(1:4,11)]=c("sido","codename","code","datetime","address")

#측정소별 한시간 단위 -> 일단위 변환 변수 
air2019$yymmdd=substr(air2019$datetime,1,8) #년월일 
...
for(i in 1:length(key)) {
  ...
  #2019-01-01 ~ 2019-12-31 일별 날짜와 merge 하여 결측값은 NA , N=365
